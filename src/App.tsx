...
function App() {
  ...
  const filteredArticles = useMemo(() => {
    return mockArticles.filter(article => {
      ...
      // Category filter
      if (filters.category !== 'All Categories' && article.category !== filters.category) {
        return false;
      }

      // Source filter
      if (filters.source !== 'All Sources' && article.source.name !== filters.source) {
        return false;
      }
  ...
  const personalizedArticles = useMemo(() => {
    if (preferences.categories.length === 0 && preferences.sources.length === 0) {
          ...
          <main className="flex-1 px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="mb-6">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Showing {personalizedArticles.length} article{personalizedArticles.length !== 1 ? 's' : ''}
                {filters.keyword && (
                  <span> for "{filters.keyword}"</span>
                )}
              </p>
            </div>
